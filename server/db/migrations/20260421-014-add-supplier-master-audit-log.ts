import type { Sequelize } from 'sequelize';

export const id = '20260421-014-add-supplier-master-audit-log';
export const name = 'add supplier master audit log';

export async function up({ sequelize }: { sequelize: Sequelize }): Promise<void> {
  await sequelize.query(`
    CREATE TABLE IF NOT EXISTS supplier_master_audit_logs (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      supplier_master_id INTEGER NOT NULL,
      action VARCHAR(64) NOT NULL,
      operator VARCHAR(255) NOT NULL DEFAULT 'system-admin',
      meta_json TEXT,
      created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
      FOREIGN KEY (supplier_master_id) REFERENCES supplier_masters(id)
    )
  `);
  await sequelize.query(`CREATE INDEX IF NOT EXISTS idx_supplier_master_audit_supplier ON supplier_master_audit_logs(supplier_master_id)`);
}

export async function down(_ctx: { sequelize: Sequelize }): Promise<void> {
  // additive / no destructive rollback for sqlite local workflow
}
