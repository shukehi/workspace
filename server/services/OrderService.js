...
function filterOrders(orders, query = {}) {
    const status = normalizeDedupeText(query.status);
    const category = normalizeDedupeText(query.category);
    const risk = normalizeDedupeText(query.risk);
    const createdDate = normalizeDedupeText(query.createdDate);
    const keyword = normalizeDedupeText(query.keyword || query.search).toLowerCase();
    const orderNo = normalizeDedupeText(query.orderNo).toLowerCase();

    return orders.filter((order) => {
        if (status) {
            if (status === 'PENDING') {
                if (!['draft', 'submitted', 'processing', 'arrived'].includes(order.status)) return false;
