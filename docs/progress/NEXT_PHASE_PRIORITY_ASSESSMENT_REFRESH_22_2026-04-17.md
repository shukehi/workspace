# Next Phase Priority Assessment Refresh 22 (2026-04-17)

> 状态：当前决策文档。
> 用途：在 `Order lifecycle normalization` 第四刀已并入 `main` 后，再次刷新下一阶段主线优先级，确保后续治理继续跟随最新热点分布。
...
这一结论已经落地，并完成：
- `stockInOrder` dependency assembly 归一到 `buildStockInOrderLifecycleDeps()`
- 分支封板并合并回 `main`
...
- `server/services/orders/order.service.ts` 当前约 **216 行**
- 已完成：
  - query helper
...
说明：
- 订单域 normalization 已连续推进三刀，并且都保持在可控范围内
- 下一步如果继续，最自然的方向会是：
  - support / read / lifecycle contracts 的 deeper unification
  - helper dependency conventions 进一步统一
...
1. create / update / stock-in wiring 已开始呈现可统一的模式
...
建议新分支方向：
- `refactor/order-lifecycle-normalization-pass5`

建议优先围绕：
1. 评估 create/update/stock-in 之间剩余最小的 normalization 公因子
...
> **normalization 阶段继续推进，并且目前仍然是收益最高的主线。**
