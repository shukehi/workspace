# Next Phase Priority Assessment Refresh 15 (2026-04-17)

> 状态：当前决策文档。
> 用途：在 `Inventory store / state pass 8` 已并入 `main` 后，再次刷新下一阶段主线优先级，确保后续治理继续跟随最新热点分布。
...
> 优先做 **Inventory store / state pass 8**

这一结论已经落地，并完成：
- location / outbound action wrapper 下沉到 `inventoryStoreFlowActions.ts`
- 分支封板并合并回 `main`
...
说明：
- store 主体已经非常接近薄壳
- 再继续做 Inventory，会明显进入更深层 partitioning / composable strategy 选择
...
> **Order lifecycle pass 11**

建议新分支方向：
- `refactor/order-lifecycle-pass11`
...
> **现在最值得继续投入的一轮优化，是回到订单生命周期主线，开始 pass 11。**
