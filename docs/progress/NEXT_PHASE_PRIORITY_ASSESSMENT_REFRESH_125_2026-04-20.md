# Next Phase Priority Assessment Refresh 125 (2026-04-20)

> 状态：当前有效。
> 触发点：`refactor/source-analysis-deeper-normalization-pass16` 已完成并并入 `main`。
...
pass 16 再次证明 source-analysis 还有清晰、低风险的小 seam：
- store shell 与 state owner 可以分离
- 分离后 state owner 更明确
...
- 一次只切一个 seam
- 优先 owner / contract / side-effect / state-application / selector / state 层收束
...
建议下一条分支：
- `refactor/source-analysis-deeper-normalization-pass17`

优先候选：
- runtime / workflow 之间更小的 contract normalization
- workflow/store exported surface 的继续收束
