# Next Phase Priority Assessment Refresh 80 (2026-04-19)

> 状态：当前决策文档。
> 用途：在 `Inventory deeper partitioning` 第十一刀已并入 `main` 后，再次刷新下一阶段主线优先级，确保后续治理继续跟随最新热点分布。
...
这一结论已经再次落地，并完成：
- receipt reverse payload type 的 canonical source 已收回到 history flow 模块
- history action / receipt composable 层不再重复定义相同 payload shape
- 分支封板并合并回 `main`
...
- `inventoryStoreFlows.ts` 当前约 **86 行**
- `inventoryStoreHistoryFlows.ts` 现在已成为 receipt reverse payload 的 canonical type owner
- `useInventoryOutboundState.ts` 仍约 **264 行**

说明：
- Inventory 线继续稳定拿到 ownership / payload source / state assembly 分层收益
- 当前最明显的剩余复杂度仍更多集中在 wrapper/composable 侧，而不是 store shell 本身
- 下一步若继续，最自然的方向会是：
  - 继续收紧 helper 与 composable / dialog 之间剩余的 ownership / binding 交叉点
...
原因：
1. deeper partitioning 第十一刀再次证明仍能稳定拿到收益
...
建议新分支方向：
- `refactor/inventory-store-deeper-partitioning-pass12`

建议动作：
1. 继续盘点 Inventory helper / composable / dialog 之间剩余的 ownership / type / binding 交叉点
