# Next Phase Priority Assessment Refresh 26 (2026-04-17)

> 状态：当前决策文档。
> 用途：在 `Order lifecycle normalization` 第八刀已并入 `main` 后，再次刷新下一阶段主线优先级，确保后续治理继续跟随最新热点分布。
...
这一结论已经落地，并完成：
- create/update shared binding types 归一到 `OrderLifecycleServiceBindings`
- 分支封板并合并回 `main`
...
说明：
- 订单域 normalization 已连续推进多刀，并开始覆盖 runtime 与 type surface 两个维度
- 下一步如果继续，最自然的方向会是：
  - query/read/support 之间更深的 contract / typing unification
  - lifecycle deps typing / shape 的进一步统一
...
1. 订单域已经开始从 wiring normalization 走向 type/contract normalization，仍有连续收益
...
建议新分支方向：
- `refactor/order-lifecycle-normalization-pass9`

建议优先围绕：
1. 评估 query/read/support 之间剩余最小的 contract / typing 公因子
...
当前之所以继续留在订单域，是因为：
- normalization 收益已经从 runtime wiring 扩展到 shared type/contract surface
- 订单域仍然保留最值得继续提纯的复杂度面
