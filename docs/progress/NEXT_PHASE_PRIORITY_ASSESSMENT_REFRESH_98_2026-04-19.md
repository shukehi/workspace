# Next Phase Priority Assessment Refresh 98 (2026-04-19)

> 状态：当前决策文档。
> 用途：在 `Inventory deeper partitioning` 第二十九刀已并入 `main` 后，再次刷新下一阶段主线优先级，确保后续治理继续跟随最新热点分布。
...
这一结论已经再次落地，并完成：
- receipt thin wrapper 已删除
- view 与测试直接依赖 receipt audit owner 与 reverse owner
- 分支封板并合并回 `main`
...
- `useInventoryLocationState.ts` 与 `useInventoryReceiptFlow.ts` 已退役
- view 层直接接入 canonical owners；thin wrapper 继续减少

说明：
- Inventory 线继续稳定拿到 ownership / payload source / result-type / control-type / query-state regrouping / reverse-state regrouping / detail-state regrouping / submit-state regrouping / audit-state regrouping / movement-detail regrouping / wrapper deletion 收益
- 当前最明显的剩余复杂度已进一步集中在少数 orchestration composable 的残余 mixed behavior concerns，而不是 store shell、view alias、或 thin wrapper
- 下一步若继续，最自然的方向会是：
  - 继续围绕 `useInventoryOutboundState.ts` / `useInventoryPageState.ts` 做小而稳的行为层 regrouping
  - 或重新评估是否该切回其它主线
...
原因：
1. deeper partitioning 第二十九刀再次证明仍能稳定拿到收益
...
建议新分支方向：
- `refactor/inventory-store-deeper-partitioning-pass30`

建议动作：
1. 继续盘点 Inventory orchestration composable 内部剩余的 ownership / type / binding 交叉点
