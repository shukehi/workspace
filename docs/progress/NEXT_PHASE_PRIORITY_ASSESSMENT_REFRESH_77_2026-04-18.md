# Next Phase Priority Assessment Refresh 77 (2026-04-18)

> 状态：当前决策文档。
> 用途：在 `Inventory deeper partitioning` 第八刀已并入 `main` 后，再次刷新下一阶段主线优先级，确保后续治理继续跟随最新热点分布。
...
这一结论已经落地，并完成：
- location payload type 的 canonical source 已收回到 flow 模块
- 分支封板并合并回 `main`
...
- `inventoryStoreState.ts` 已聚焦 state assembly
- `inventoryStoreCollections.ts` 已承接 raw collection refs
- `inventoryStoreMetaState.ts` 已承接 loading / totals / page state
- `inventoryStoreDerivedState.ts` 已承接 sorted / filtered / active derived selectors
- outbound / location payload type source 已归回 flow 层

说明：
- Inventory 线继续稳定拿到结构收益
- collection ownership、meta ownership、derived ownership、action composition ownership、payload type ownership 都在变清晰
- 下一步若继续，最自然的方向会是：
  - 继续收紧 helper 之间剩余的 ownership / binding 交叉点
  - 或进入更深一层的 composable regrouping 讨论
...
原因：
1. deeper partitioning 第八刀再次证明仍能稳定拿到收益
...
建议新分支方向：
- `refactor/inventory-store-deeper-partitioning-pass9`
