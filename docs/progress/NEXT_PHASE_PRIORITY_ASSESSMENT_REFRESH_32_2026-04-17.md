# Next Phase Priority Assessment Refresh 32 (2026-04-17)

> 状态：当前决策文档。
> 用途：在 `Order lifecycle normalization` 第十四刀已并入 `main` 后，再次刷新下一阶段主线优先级，确保后续治理继续跟随最新热点分布。
...
这一结论已经落地，并完成：
- shared lifecycle dependency-core contracts 归一到 `order.service.contracts.ts`
- 分支封板并合并回 `main`
...
- 已完成：
  ...
  - shared order-service contracts（serialization / summary-facet / lookup / transaction / persistence / duplicate / idempotency / lifecycle deps core）

说明：
- 订单域 normalization 已连续推进多刀，并且 contracts 层已进一步成型
- 下一步如果继续，最自然的方向会是：
  - contracts 层的 naming / organization consolidation
  - shared contract 之间更高层的分层梳理
...
建议新分支方向：
- `refactor/order-lifecycle-normalization-pass15`

建议优先围绕：
1. 评估 contracts 层里剩余最小的 naming / organization consolidation target
