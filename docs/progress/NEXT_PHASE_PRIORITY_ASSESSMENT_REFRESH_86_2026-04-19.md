# Next Phase Priority Assessment Refresh 86 (2026-04-19)

> 状态：当前决策文档。
> 用途：在 `Inventory deeper partitioning` 第十七刀已并入 `main` 后，再次刷新下一阶段主线优先级，确保后续治理继续跟随最新热点分布。
...
这一结论已经再次落地，并完成：
- `Inventory.vue` 中的本地 alias 层已删除
- 模板直接绑定 owner composable 提供的 state / action
- 分支封板并合并回 `main`
...
- `Inventory.vue` 不再保留本地 alias layer
- `useInventoryOutboundState.ts` 仍约 **269 行**

说明：
- Inventory 线继续稳定拿到 ownership / payload source / result-type / control-type / helper deletion / view-binding 直连收益
- 当前最明显的剩余复杂度仍更多集中在 composable 本身，而不是 store shell 或 view alias 层
- 下一步若继续，最自然的方向会是：
  - 继续收紧 `useInventoryOutboundState.ts` / `useInventoryReceiptFlow.ts` 等 composable 内部剩余的 ownership / regrouping 点
  - 或围绕 outbound/location/receipt 交互面做更小的 composable 拆分
...
原因：
1. deeper partitioning 第十七刀再次证明仍能稳定拿到收益
...
建议新分支方向：
- `refactor/inventory-store-deeper-partitioning-pass18`
