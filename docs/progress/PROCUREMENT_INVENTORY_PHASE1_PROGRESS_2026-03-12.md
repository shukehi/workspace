...
1. 已支持部分入库、多次入库与部分撤销入库，但独立的入库记录详情页仍未实现，当前为详情抽屉
3. 库存页入库记录已完成服务端筛选、服务端分页与页大小切换，但仍缺更细的分页体验优化
4. 批量入库暂时禁用，尚未提供批量场景下的明细选择方案
5. 订单级 `stocked_in_*` 仍只表达“整单完成入库”，若后续要展示最近一次入库时间需新增独立字段或直接读流水
6. 采购页当前依赖浏览器 `storage` 事件感知库存页撤销，后续可考虑抽成统一事件总线
7. 入库记录导出当前仍是平面 CSV，尚未包含“原始流水 -> 撤销子流水”的层级关系
