# Next Phase Priority Assessment Refresh 36 (2026-04-17)

> 状态：当前决策文档。
> 用途：在 `Order lifecycle normalization` 第十八刀已并入 `main` 后，再次刷新下一阶段主线优先级，确保后续治理继续跟随最新热点分布。
...
这一结论已经落地，并完成：
- contracts 层的 lifecycle alias consolidation
- 分支封板并合并回 `main`
...
- 已完成：
  ...
  - shared order-service contracts（serialization / summary-facet / lookup / transaction / persistence / duplicate / idempotency / allocation / lifecycle core / runtime aliases / lifecycle aliases）

说明：
- 订单域 normalization 已连续推进多刀，并且 contracts 层的命名与组织继续收束
- 下一步如果继续，最自然的方向会是：
  - contracts 层剩余最小的 consolidation / cleanup
  - helper deps shape 命名的最终提纯
...
1. contracts 层仍在持续成型，并且还有剩余可收口的 bounded 目标
...
建议新分支方向：
- `refactor/order-lifecycle-normalization-pass19`

建议优先围绕：
1. 评估 contracts 层里剩余最小的 consolidation / naming cleanup target
