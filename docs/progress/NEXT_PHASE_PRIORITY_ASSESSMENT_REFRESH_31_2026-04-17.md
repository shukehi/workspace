# Next Phase Priority Assessment Refresh 31 (2026-04-17)

> 状态：当前决策文档。
> 用途：在 `Order lifecycle normalization` 第十三刀已并入 `main` 后，再次刷新下一阶段主线优先级，确保后续治理继续跟随最新热点分布。
...
这一结论已经落地，并完成：
- create/update shared core lifecycle contract fragment 归一到 `OrderCoreLifecycleBindings`
- 分支封板并合并回 `main`
...
- 已完成：
  ...
  - shared order-service contracts（serialization / summary-facet / lookup / transaction / persistence / duplicate / idempotency / core lifecycle）

说明：
- 订单域 normalization 已连续推进多刀，并且 contracts 层已经形成较完整的共享骨架
- 下一步如果继续，最自然的方向会是：
  - shared contracts 的命名/组织进一步 consolidation
  - deps shape / lifecycle surface 的更高层统一
...
建议新分支方向：
- `refactor/order-lifecycle-normalization-pass14`

建议优先围绕：
1. 评估 shared contracts 层中剩余最小的 consolidation target
