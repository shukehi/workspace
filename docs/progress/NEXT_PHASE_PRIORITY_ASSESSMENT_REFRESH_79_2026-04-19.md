# Next Phase Priority Assessment Refresh 79 (2026-04-19)

> 状态：当前决策文档。
> 用途：在 `Inventory deeper partitioning` 第十刀已并入 `main` 后，再次刷新下一阶段主线优先级，确保后续治理继续跟随最新热点分布。
...
这一结论已经再次落地，并完成：
- location dialog 的 emit payload type 已收回到 flow 模块
- dialog / composable / flow 三层不再重复定义相同 payload shape
- 分支封板并合并回 `main`
...
- `useInventoryOutboundState.ts` 当前约 **264 行**
- `InventoryLocationDialog.vue` 已不再重复声明 location payload shape
...
- 下一步若继续，最自然的方向会是：
  ...
  - 或围绕 outbound/location 交互面做更小的 regrouping
...
原因：
1. deeper partitioning 第十刀再次证明仍能稳定拿到收益
...
建议新分支方向：
- `refactor/inventory-store-deeper-partitioning-pass11`
