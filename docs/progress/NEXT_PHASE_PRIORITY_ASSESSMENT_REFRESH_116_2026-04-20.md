# Next Phase Priority Assessment Refresh 116 (2026-04-20)

> 状态：当前有效。
> 触发点：`refactor/source-analysis-deeper-normalization-pass7` 已完成并并入 `main`。
...
pass 7 再次证明 source-analysis 还有清晰、低风险的小 seam：
- request orchestration 和 fetch/history error normalization 可以分离
- 分离后 request-error owner 更明确
...
建议下一条分支：
- `refactor/source-analysis-deeper-normalization-pass8`

优先候选：
- runtime / workflow 之间更小的 contract normalization
- workflow 中 remaining analyze invocation / result delegation 的再收束
