# Next Phase Priority Assessment Refresh 93 (2026-04-19)

> 状态：当前决策文档。
> 用途：在 `Inventory deeper partitioning` 第二十四刀已并入 `main` 后，再次刷新下一阶段主线优先级，确保后续治理继续跟随最新热点分布。
...
这一结论已经再次落地，并完成：
- movement detail state 已收回到独立 owner helper
- page state composable 不再内联整组 movement detail refs/behavior
- 分支封板并合并回 `main`
...
- `useInventoryReceiptFlow.ts` 更接近 audit-oriented orchestration shell
- `useInventoryOutboundState.ts` 更接近 non-reverse / non-detail outbound behavior orchestration shell
- `useInventoryPageState.ts` 更接近 page-level orchestration shell

说明：
- Inventory 线继续稳定拿到 ownership / payload source / result-type / control-type / query-state regrouping / reverse-state regrouping / detail-state regrouping / audit-state regrouping / movement-detail regrouping 收益
- 当前最明显的剩余复杂度进一步集中在少数 composable 内部剩余 mixed behavior concerns，而不是 store shell、view alias、或 helper duplication
- 下一步若继续，最自然的方向会是：
  - 继续围绕 `useInventoryOutboundState.ts` / `useInventoryReceiptFlow.ts` / `useInventoryPageState.ts` 做小而稳的行为层 regrouping
  - 或处理 location side 的残余 owner/binding 交叉点
...
原因：
1. deeper partitioning 第二十四刀再次证明仍能稳定拿到收益
...
建议新分支方向：
- `refactor/inventory-store-deeper-partitioning-pass25`
