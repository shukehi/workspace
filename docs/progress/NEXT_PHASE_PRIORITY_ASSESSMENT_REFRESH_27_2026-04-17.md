# Next Phase Priority Assessment Refresh 27 (2026-04-17)

> 状态：当前决策文档。
> 用途：在 `Order lifecycle normalization` 第九刀已并入 `main` 后，再次刷新下一阶段主线优先级，确保后续治理继续跟随最新热点分布。
...
这一结论已经落地，并完成：
- read/query shared contracts 归一到 `order.service.contracts.ts`
- 分支封板并合并回 `main`
...
- 已完成：
  ...
  - shared order-service contracts

说明：
- 订单域 normalization 已连续推进多刀，并且已经开始形成 shared contract 层
- 下一步如果继续，最自然的方向会是：
  - support/read/query contracts 的 deeper typing unification
  - lifecycle deps shape 的更高层统一
...
1. 订单域已经开始从 shared bindings 走向 shared contracts，这条线仍有连续收益
...
建议新分支方向：
- `refactor/order-lifecycle-normalization-pass10`

建议优先围绕：
1. 评估 support/read/query/lifecycle 之间剩余最小的 contract / typing 公因子
...
当前之所以继续留在订单域，是因为：
- runtime wiring 与 shared contract 两层统一都已证明有价值
- 订单域仍然保留最值得继续提纯的复杂度面
