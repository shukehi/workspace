# Next Phase Priority Assessment Refresh 25 (2026-04-17)

> 状态：当前决策文档。
> 用途：在 `Order lifecycle normalization` 第七刀已并入 `main` 后，再次刷新下一阶段主线优先级，确保后续治理继续跟随最新热点分布。
...
这一结论已经落地，并完成：
- `getPaginatedOrders` shared query bindings 归一到 `buildOrderQueryBindings()`
- 分支封板并合并回 `main`
...
说明：
- 订单域 normalization 已连续推进多刀，并且逐步从 per-path wiring 走向 shared surface 契约统一
- 下一步如果继续，最自然的方向会是：
...
1. 订单域已经开始出现跨 query/read/support 的连续 normalization 收益
...
建议新分支方向：
- `refactor/order-lifecycle-normalization-pass8`
...
当前之所以继续留在订单域，是因为：
- wiring 统一已经开始从 lifecycle 内部收益转向 shared surface 契约收益
- 订单域仍然保留最值得继续提纯的复杂度面
