# Next Phase Priority Assessment Refresh 35 (2026-04-17)

> 状态：当前决策文档。
> 用途：在 `Order lifecycle normalization` 第十七刀已并入 `main` 后，再次刷新下一阶段主线优先级，确保后续治理继续跟随最新热点分布。
...
这一结论已经落地，并完成：
- shared allocation contract fragment 归一到 `order.service.contracts.ts`
- 分支封板并合并回 `main`
...
- 已完成：
  ...
  - create helper（含 deps builder + shared binding types + shared allocation contract）
  - update helper（含 deps builder + shared binding types + shared core lifecycle contract）
  - delete helper
  ...
  - shared order-service contracts（serialization / summary-facet / lookup / transaction / persistence / duplicate / idempotency / allocation / lifecycle deps core / alias 组织）

说明：
- 订单域 normalization 已连续推进多刀，并且 contracts 层的共享骨架继续细化
- 下一步如果继续，最自然的方向会是：
  - contracts 层命名/组织的进一步收束
  - helper deps shape 的最终精简
...
1. contracts 层仍在持续成型，并且还在带来 bounded 收益
...
建议新分支方向：
- `refactor/order-lifecycle-normalization-pass18`

建议优先围绕：
1. 评估 contracts 层里剩余最小的命名/组织 consolidation target
