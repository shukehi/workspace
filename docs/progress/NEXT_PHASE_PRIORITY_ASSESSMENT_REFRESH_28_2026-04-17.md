# Next Phase Priority Assessment Refresh 28 (2026-04-17)

> 状态：当前决策文档。
> 用途：在 `Order lifecycle normalization` 第十刀已并入 `main` 后，再次刷新下一阶段主线优先级，确保后续治理继续跟随最新热点分布。
...
这一结论已经落地，并完成：
- shared lookup contracts 归一到 `order.service.contracts.ts`
- 分支封板并合并回 `main`
...
- 已完成：
  ...
  - shared order-service contracts（serialization / summary-facet / lookup）

说明：
- 订单域 normalization 已连续推进多刀，并且 shared contract 层已经开始成型
- 下一步如果继续，最自然的方向会是：
  - shared contract typing 的继续细化
...
1. 订单域已经开始形成 shared contract 层，继续推进仍有连续收益
...
建议新分支方向：
- `refactor/order-lifecycle-normalization-pass11`
...
当前之所以继续留在订单域，是因为：
- wiring → bindings → types → shared contracts 的统一还在持续产出价值
- 订单域仍然保留最值得继续提纯的复杂度面
