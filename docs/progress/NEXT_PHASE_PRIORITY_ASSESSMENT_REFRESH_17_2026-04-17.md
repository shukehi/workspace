# Next Phase Priority Assessment Refresh 17 (2026-04-17)

> 状态：当前决策文档。
> 用途：在 `Inventory store / state pass 9` 已并入 `main` 后，再次刷新下一阶段主线优先级，确保后续治理继续跟随最新热点分布。
...
> 优先做 **Inventory store / state pass 9**

这一结论已经落地，并完成：
- action composition surface 下沉到 `inventoryStoreActions.ts`
- 分支封板并合并回 `main`
...
- `src/stores/useInventoryStore.ts` 当前约 **52 行**
- 已完成：
  - pagination helper
  - core flow helper
  - core actions helper
...
### 第一优先：Order lifecycle pass 12
**推荐重新回到订单生命周期主线。**

原因：

1. Inventory store 在连续多轮 helper / action / state 下沉后，已经进入更深层设计判断阶段
...
> **Order lifecycle pass 12**

建议新分支方向：
- `refactor/order-lifecycle-pass12`
...
> **现在最值得继续投入的一轮优化，是回到订单生命周期主线，开始 pass 12。**
