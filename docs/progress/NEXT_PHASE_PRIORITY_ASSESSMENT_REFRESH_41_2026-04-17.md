# Next Phase Priority Assessment Refresh 41 (2026-04-17)

> 状态：当前决策文档。
> 用途：在 `Order lifecycle normalization` 第二十三刀已并入 `main` 后，再次刷新下一阶段主线优先级，确保后续治理继续跟随最新热点分布。
...
这一结论已经落地，并完成：
- stock-in helper 的 shared contract shape 归一到 contracts 层
- 分支封板并合并回 `main`
...
- 已完成：
  ...
  - stockIn helper（含 entry wrapper + deps builder + shared contracts）
  - support helper（含 shared lifecycle bindings）
  - shared order-service contracts（serialization / summary-facet / lookup / transaction / persistence / duplicate / idempotency / allocation / lifecycle core / runtime-deps alias / read-query deps alias / stock-in helper contracts）

说明：
- 订单域 normalization 已连续推进多刀，并且 contracts 层对 helper surface 的覆盖面持续提升
- 下一步如果继续，最自然的方向会是：
  - contracts 层剩余最小的 naming / organization cleanup
  - 可能的 helper deps shape 最终统一
...
1. shared contracts 层继续扩展，并仍然带来 bounded 收益
...
建议新分支方向：
- `refactor/order-lifecycle-normalization-pass24`
...
当前之所以继续留在订单域，是因为：
- shared contracts 层继续成熟并持续带来 bounded 价值
- 订单域仍然保留最值得继续提纯的复杂度面
