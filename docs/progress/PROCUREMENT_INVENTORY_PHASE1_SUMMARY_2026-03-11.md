...
1. 批量入库仍然禁用，尚未设计批量明细选择方案
2. 入库记录当前为详情抽屉，尚未拆成独立详情页
3. CSV 导出仍是平面结构，未表达“原始流水 -> 撤销子流水”层级
4. 采购页与库存页之间的联动刷新仍依赖 `storage` 事件
5. 尚缺一轮完整人工回归，验证真实业务操作链路
