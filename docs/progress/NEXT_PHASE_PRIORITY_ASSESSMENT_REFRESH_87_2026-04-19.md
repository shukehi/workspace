# Next Phase Priority Assessment Refresh 87 (2026-04-19)

> 状态：当前决策文档。
> 用途：在 `Inventory deeper partitioning` 第十八刀已并入 `main` 后，再次刷新下一阶段主线优先级，确保后续治理继续跟随最新热点分布。
...
这一结论已经再次落地，并完成：
- outbound query/filter state 已收回到独立 owner helper
- outbound behavior composable 不再内联整组 query/filter refs
- 分支封板并合并回 `main`
...
- `useInventoryOutboundState.ts` 已进一步收口为 outbound behavior orchestration shell
- `useInventoryOutboundQueryState.ts` 已成为 outbound query/filter/pagination/derived state 的 canonical owner

说明：
- Inventory 线继续稳定拿到 ownership / payload source / result-type / control-type / helper deletion / query-state regrouping 收益
- 当前最明显的剩余复杂度开始更集中在 composable 内部剩余 mixed concerns，而不是 store shell 或 view alias 层
- 下一步若继续，最自然的方向会是：
  - 继续围绕 `useInventoryOutboundState.ts` / `useInventoryReceiptFlow.ts` 做小而稳的内部 regrouping
  - 或处理 location / receipt 侧残余的 owner/binding 交叉点
...
原因：
1. deeper partitioning 第十八刀再次证明仍能稳定拿到收益
...
建议新分支方向：
- `refactor/inventory-store-deeper-partitioning-pass19`
