# Next Phase Priority Assessment Refresh 24 (2026-04-17)

> 状态：当前决策文档。
> 用途：在 `Order lifecycle normalization` 第六刀已并入 `main` 后，再次刷新下一阶段主线优先级，确保后续治理继续跟随最新热点分布。
...
这一结论已经落地，并完成：
- `getAllOrders` / `getOrderById` shared read bindings 归一到 `buildOrderReadBindings()`
- 分支封板并合并回 `main`
...
- `server/services/orders/order.service.ts` 当前约 **218 行**
- 已完成：
  - query helper
...
说明：
- 订单域 normalization 已连续推进多刀，并且仍保持在可控范围内
- 下一步如果继续，最自然的方向会是：
  - query/read/support contracts 的 deeper unification
  - lifecycle deps typing / shape 进一步统一
...
1. query/read/support 与 create/update/stock-in 之间已经开始形成可继续统一的 contract 面
...
建议新分支方向：
- `refactor/order-lifecycle-normalization-pass7`
...
当前之所以继续留在订单域，是因为：
- wiring 统一已经从单路径收益转向多路径 contract 收益
- 订单域仍然保留最值得继续提纯的复杂度面
