# Next Phase Priority Assessment Refresh 16 (2026-04-17)

> 状态：当前决策文档。
> 用途：在 `Order lifecycle pass 11` 已并入 `main` 后，再次刷新下一阶段主线优先级，确保后续治理继续跟随最新热点分布。
...
> 优先做 **Order lifecycle pass 11**

这一结论已经落地，并完成：
- `getOrderById` read helper 下沉到 `order.service.read.ts`
- 分支封板并合并回 `main`
...
### Order lifecycle
- `server/services/orders/order.service.ts` 当前约 **227 行**
- 已完成：
  - query helper
...
- `src/stores/useInventoryStore.ts` 当前约 **148 行**
- 已完成：
  - pagination helper
  - core flow helper
  - core actions helper
...
说明：
- store 主体已经非常接近薄壳，但相比订单域更适合继续切一刀小而稳的剩余 surface
- 下一步仍可围绕：
  - deeper partitioning
  - remaining state/action surface normalization
...
### 第一优先：Inventory store / state pass 9
...
> **Inventory store / state pass 9**

建议新分支方向：
- `refactor/inventory-store-pass9`

建议优先围绕：
1. 继续处理 store 中剩余的 action / state surface
2. 评估最小且仍行为保持的下一刀
3. 保持小步、可验证的节奏
...
> **现在最值得继续投入的一轮优化，是回到 Inventory store / state 主线，开始 pass 9。**
