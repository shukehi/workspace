# Next Phase Priority Assessment Refresh 23 (2026-04-17)

> 状态：当前决策文档。
> 用途：在 `Order lifecycle normalization` 第五刀已并入 `main` 后，再次刷新下一阶段主线优先级，确保后续治理继续跟随最新热点分布。
...
这一结论已经落地，并完成：
- create/update shared lifecycle bindings 归一到 `buildOrderLifecycleBindings()`
- 分支封板并合并回 `main`
...
说明：
- 订单域 normalization 已连续推进多刀，并且依然保持在可控范围内
- 下一步如果继续，最自然的方向会是：
  - support/read/query contracts 的 deeper unification
...
1. shared bindings 统一已经开始形成跨 helper 的连续收益
...
建议新分支方向：
- `refactor/order-lifecycle-normalization-pass6`

建议优先围绕：
1. 评估 support/read/query 之间剩余最小的 normalization 公因子
