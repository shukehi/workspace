# Next Phase Priority Assessment Refresh 100 (2026-04-19)

> 状态：当前决策文档。
> 用途：在 `Inventory deeper partitioning` 第三十一刀已并入 `main` 后，再次刷新下一阶段主线优先级，确保后续治理继续跟随最新热点分布。
...
这一结论已经再次落地，并完成：
- inventory page list behavior 已收回到独立 owner helper
- page state composable 不再内联整组 load/export/filter-watch/coherence logic
- 分支封板并合并回 `main`
...
- `useInventoryPageState.ts` 更接近 page orchestration shell
- `useInventoryOutboundState.ts` 更接近 outbound orchestration shell
...
- 当前最明显的剩余复杂度已进一步集中在极少数 orchestration composable 的残余 mixed behavior concerns，而不是 store shell、view alias、或 thin wrapper
- 下一步若继续，最自然的方向会是：
  - 继续围绕 `useInventoryOutboundState.ts` 与 `useInventoryPageState.ts` 做更小的 orchestration regrouping
  - 或重新评估是否该切回其它主线
...
原因：
1. deeper partitioning 第三十一刀再次证明仍能稳定拿到收益
...
建议新分支方向：
- `refactor/inventory-store-deeper-partitioning-pass32`
