...
> **当前状态：阶段一、二、三已高质量完成 (Done)，后端核心重构目标达成。**
> **最后更新：2026-03-14**

## 1. 总体目标
*   **消除混合态**：将 `server/` 目录下伪装成 `.ts` 的 CommonJS 代码彻底转化为标准的 ESM (TypeScript)。 [DONE]
*   **架构一致性**：全面推广 `Repository` 模式，实现业务逻辑与数据访问的解耦。 [DONE]
*   **功能补完**：实现物料匹配的模糊搜索与别名逻辑，提升系统智能化程度。 [DONE]
*   **错误治理**：解耦巨型错误转换逻辑，提升系统的可扩展性。 [DONE]
