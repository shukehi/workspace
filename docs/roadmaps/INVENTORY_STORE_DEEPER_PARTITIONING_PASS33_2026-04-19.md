# Inventory Store Deeper Partitioning Pass 33 (2026-04-19)

> 状态：当前执行计划。
> 分支：`refactor/inventory-store-deeper-partitioning-pass33`
> 范围：在 outbound query/list/reverse/detail/submit state 已各自独立拥有后，继续删除 `useInventoryOutboundState.ts` 这一层薄组合包装，而不改变页面行为或交互逻辑。

---

## 1. 背景

当前 outbound 相关 state 已分别拥有明确 owner：
...
而 `useInventoryOutboundState.ts` 仅负责把这些 owner 拼在一起后再转发给 `Inventory.vue`，已经成为一层纯中转包装。
...
- 删除 `useInventoryOutboundState.ts`
- 让 `Inventory.vue` 直接依赖 outbound canonical owners
...
1. 不改 Inventory 页面行为
2. 不改 outbound 列表、detail、submit、reverse 语义
3. 不做 multi-store 拆分
...
- `src/features/inventory/composables/useInventoryOutboundQueryState.ts`
...
1. `useInventoryOutboundState.ts` 被删除
2. `Inventory.vue` 直接组合 outbound owners
3. 页面行为不变
4. 定向测试与全量门禁继续通过
