# Source-analysis Deeper Normalization Pass 18 (2026-04-20)

> 状态：当前执行计划。
> 分支：`refactor/source-analysis-deeper-normalization-pass18`
> 范围：在不改变 Source 页面行为的前提下，把 view 对 source store 的 consumer-facing reads 收束到 page-state owner。

---

## 1. 背景

当前 `src/views/Source.vue` 仍直接读取：
- `store.loading`
- `store.hasOrder`
- `store.currentOrder`
- `store.orderItems`

而 `useSourcePageState()` 已经是页面层的 owner 入口，这里仍有一层 consumer-surface split：
- 页面行为状态在 composable
- 页面展示读取却仍散落在 view 里
...
只做一个切口：
- 让 `useSourcePageState()` 承接 Source view 当前依赖的 consumer-facing store reads
...
目标：
- Source view 对 store 的 consumer-facing reads 进入 page-state owner
- view 只组合 page-state 输出与 UI 组件
...
1. `Source.vue` 不再直接引用 `store.hasOrder/currentOrder/orderItems/loading`
2. `useSourcePageState()` 暴露这些 consumer-facing values
3. 页面行为不变
4. 定向测试与全量门禁继续通过
