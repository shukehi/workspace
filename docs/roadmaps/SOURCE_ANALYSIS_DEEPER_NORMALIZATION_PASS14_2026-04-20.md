# Source-analysis Deeper Normalization Pass 14 (2026-04-20)

> 状态：当前执行计划。
> 分支：`refactor/source-analysis-deeper-normalization-pass14`
> 范围：在不改变 source-analysis 行为的前提下，把 currentOrder-based selectors 从 store shell 中提炼为独立 owner。
...
它们都是围绕 `currentOrder` 的稳定派生视图，已形成一个清晰的小 seam。
...
只做一个切口：
- 提炼 source-order selectors helper
- 让 `useSourceStore()` 更接近 state / workflow / derived-state composition shell
...
- `src/features/source-analysis/composables/useSourceOrderSelectors.ts`（新增）
- `tests/source-order-selectors.test.ts`（新增）
- `docs/README.md`

目标：
- currentOrder 派生视图拥有独立 owner
- store 只组合 state / workflow / selector / derived-state owners
...
1. `hasOrder` / `orderItems` 不再内联在 `useSourceStore.ts`
