# Source-analysis Deeper Normalization Pass 9 (2026-04-20)

> 状态：当前执行计划。
> 分支：`refactor/source-analysis-deeper-normalization-pass9`
> 范围：在不改变 source-analysis 行为的前提下，把 workflow clear/reset side effects 从 orchestration 中提炼为独立 owner。

---

## 1. 背景

当前 `createSourceOrderWorkflow()` 的 `clear()` 仍同时承担：
- currentOrder clear
- analysis state clear
- error reset
- snapshot clear side effect

这已经形成一个独立的小 seam：clear/reset side-effect contract 应先有自己的 owner，再由 workflow 只负责 orchestration surface。
...
只做一个切口：
- 提炼 source-order clear/reset applier helper
- 让 workflow 不再内联 clear/reset side effects
...
2. 不改 fetch / history / cache / request / analyze 语义
3. 不改 store / view 结构
...
- `src/features/source-analysis/services/sourceOrderClearApplier.ts`（新增）
- `tests/source-order-clear-applier.test.ts`
- `tests/source-store-workflow.test.ts`
- `docs/README.md`

目标：
- clear/reset owner 独立
...
1. clear side effects 不再内联在 workflow 中
