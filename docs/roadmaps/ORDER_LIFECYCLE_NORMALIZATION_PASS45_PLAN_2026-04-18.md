# Order Lifecycle Normalization Pass 45 Plan (2026-04-18)

> 状态：当前执行计划。
> 分支：`refactor/order-lifecycle-normalization-pass45`
> 范围：在 normalization 阶段继续收束 create/update/support 侧的 idempotency-reserve alias，但不改变 API contract 或业务语义。
...
- 去掉 `OrderIdempotencyReserveBinding`
- 让 create/update/support helper 直接依赖 `reserveIdempotencyKey` 的显式函数签名
...
目标：
- create/update/support helper 直接写出 `reserveIdempotencyKey` 的函数签名
- 只移除没有新增语义的 idempotency-reserve alias
...
1. `OrderIdempotencyReserveBinding` 不再存在
2. 行为不变
3. 定向测试继续通过
4. 全量门禁保持全绿
