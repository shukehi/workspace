...
5. 采购页单单入口已切到明细入库弹窗，批量入库也已恢复为逐单排队处理模式，避免重新退回旧的整单一键入库语义
...
8. 对历史兼容数据，若 `ordered_quantity <= 0` 且 `quantity > 0`，前后端都必须回退使用 `quantity` 参与剩余待入库判断与入库计算
...
2. 批量入库逻辑，不能再直接假设 `arrived -> completed`；当前采用逐单排队的明细入库流程，后续再考虑统一的批量明细选择方案
