...
27. 采购管理页已切换为服务端分页 + 服务端筛选，页码通过 URL 同步，表格改为手动分页模式，默认每页 20 条，并支持页大小切换
28. 采购管理页导出已改为按当前查询条件导出全量结果，不再只导出当前页
...
13. 先临时禁用批量入库入口，避免旧整单语义与新明细入库弹窗冲突；后续已恢复为逐单排队的明细入库流程
...
24. 采购管理页不再一次性拉取全量订单，改为 `/orders` 服务端分页返回结构，避免前端全量加载后再本地分页
25. 修复采购管理页服务端分页翻页后仍保留旧页选中态的问题，避免批量操作和“导出所选”误作用于上一页订单
26. 修复采购管理页页大小切回默认 `20` 时被旧 `store.ordersPageSize` 回写成 `50` 的问题，页大小选择现在可在 `20 / 50 / 100` 之间稳定切换
27. 采购订单相关页面的主标题已统一为 `颐家工贸 (  总厂  ) 采购订单`，避免预览、编辑、打印与文档模型使用不同抬头
...
2. 库存页入库记录已完成服务端筛选、服务端分页与页大小切换；采购页也已切换到服务端分页、默认 20 条/页，并已支持页大小切换与按查询条件全量导出
3. 批量入库已恢复为逐单排队处理模式，但仍未提供批量场景下的统一明细矩阵选择界面
