...
const MappingWorkflow = require('../server/services/mappings');
const {
  PROFILE_CODES,
  REVISION_STATES,
} = require('../server/services/mappings/mapping.constants');
...
test('mapping workflow skeleton: enforces single draft, optimistic lock, publish switch and rollback-to-draft flow', async () => {
  ...
  assert.deepEqual(
    revisions.revisions.map((item) => [item.revision, item.state]),
    [
  ...
  const profile = await MappingProfile.findOne({ where: { profile_code: PROFILE_CODES.PACKAGING } });
  ...
  assert.deepEqual(
    revisionRows.map((item) => item.state),
    [
