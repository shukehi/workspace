...
test('procurement bulk action guard: supports restoring selected orders to draft', () => {
  ...
  const bulkActions = read('src/features/procurement/composables/useProcurementBulkActions.ts')
  const stockInQueue = read('src/features/procurement/composables/useStockInQueue.ts')
  ...
  assert.match(bulkActions, /当前所选订单不能批量设为/)
  assert.match(bulkActions, /当前所选订单不能批量登记到货/)
  assert.match(bulkActions, /批量到货部分完成/)
  assert.match(bulkActions, /hasRemainingStockInItems/)
  assert.match(bulkActions, /order\.status === 'arrived' && hasRemainingStockInItems\(order\)/)
  assert.match(page, /openStockInQueue/)
  assert.match(stockInQueue, /批量入库已中止/)
})
...
test('procurement stock-in guard: arrived orders use detail stock-in dialog', () => {
  ...
  const stockInQueue = read('src/features/procurement/composables/useStockInQueue.ts')
  const stockInDraft = read('src/features/procurement/stockInDraft.ts')

  assert.match(page, /import ProcurementStockInDialog/)
  assert.match(page, /useInventoryStore/)
  // stockIn logic extracted into useStockInQueue composable
  assert.match(stockInQueue, /import \{ hasRemainingStockInItems \} from '@\/features\/procurement\/stockInEligibility'/)
  assert.match(stockInQueue, /const stockInOrder = ref<Order \| null>\(null\)/)
  assert.match(stockInQueue, /const stockInDialogOpen = ref\(false\)/)
  assert.match(stockInQueue, /const stockInSaving = ref\(false\)/)
  assert.match(stockInQueue, /const stockInQueue = ref<Order\[\]>\(\[\]\)/)
  assert.match(stockInQueue, /const stockInQueueIndex = ref\(0\)/)
  assert.match(stockInQueue, /const openStockInDialog = \(order: Order\)/)
  assert.match(stockInQueue, /const openStockInQueue = \(orders: Order\[\]\)/)
  assert.match(page, /handleStockInDialogOpenChange/)
  assert.match(stockInQueue, /if \(!hasRemainingStockInItems\(order\)\)/)
  assert.match(stockInQueue, /当前订单没有可继续入库的明细/)
  assert.match(page, /onStockIn: openStockInDialog/)
  assert.match(page, /<ProcurementStockInDialog/)
  assert.match(page, /:warehouses="inventoryStore\.warehouses"/)
  assert.match(page, /:locations="inventoryStore\.activeLocations"/)
  ...
  assert.match(stockInQueue, /title: isCompleted \? '入库完成，进入下一单' : '部分入库成功，进入下一单'/)
  assert.match(stockInQueue, /title: queueActive/)
  assert.match(stockInQueue, /'批量入库流程已完成'/)
  assert.match(stockInQueue, /'批量入库已完成'/)
  assert.match(stockInQueue, /张已完成入库，\$\{pendingCount\} 张仍有明细待入库/)
  ...
  assert.match(dialog, /selectedWarehouseId/)
  assert.match(dialog, /selectedLocationId/)
  assert.match(dialog, /入库库位/)
  assert.match(dialog, /warehouse_id:/)
  assert.match(dialog, /location_id:/)
  assert.match(dialog, /批量入库第/)
  assert.match(dialog, /全入/)
  assert.match(dialog, /全入当前单并下一单/)
  assert.match(dialog, /handleSubmitAllRemaining/)
  assert.match(stockInDraft, /item_key/)
  assert.match(stockInDraft, /remaining/)
  assert.match(stockInDraft, /quantity: ''/)
  assert.match(stockInDraft, /buildFullStockInItems/)
})
