...
test('layout guard: shared order sheet owns procurement table markup', () => {
  ...
  assert.match(orderSheet, /resolveAggregateQuantityColumnWidth/)
  assert.match(orderSheet, /distributeAggregateQuantityColumnWidth/)
  assert.match(orderSheet, /qtyLeft: props\.defaultWidths\.qtyLeft \|\| 72/)
  assert.match(orderSheet, /qtyRight: props\.defaultWidths\.qtyRight \|\| 72/)
})
