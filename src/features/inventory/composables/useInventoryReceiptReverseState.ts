...
export function useInventoryReceiptReverseState(options: {
  reverseReceipt: (id: number, payload: InventoryReceiptReversePayload) => Promise<InventoryReceipt>;
  ...
  async function confirmReverseReceipt(orderNo = '') {
    ...
    try {
      await options.reverseReceipt(reverseReceiptTarget.value.id, {
