...
- 锁具备注只使用命中映射的 `remark`
- 锁叉支持高门基础尺寸规则：5/7cm 在 `>= 2200` 时切换高门尺寸且增量全部加到上头，9cm 在 `>= 2210` 时切换到高门尺寸
...
- 手动录入单不参与这条自动防重规则
- 手动录入单保存时要求 `客户名称 / 供应商 / 交货日期 / 有效明细` 完整；空白占位行不会被当成有效明细
- `arrived / completed` 的手动录入单仍只允许修改锁定状态下开放的字段，不会因为历史脏明细而阻断备注或交期更新
